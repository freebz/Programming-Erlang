...
make_tables() ->
    ...
    io:format("Ets ordered Set size=~p time/tirgram=~p~n" ,[FileSize1/Ntri,
							    Micro2/N]),
...
timer_tests() ->
    ...
    time_lookup_ets_set("Tes set" , "trigramsS.tab" ),
...
time_lookup_module_sets() ->
    ...
    io:format("Module set lookup=~p micro secodns~n" ,[M/Size]).
...
%% 단어를 뒤져 |r|n(//역슬래시예요)을 찾는다.
%% 두 번째 인수는 (뒤집힌) 단어이며 따라서
%% |r|n을 찾거나 또는 문자에서 벗어났을 때 뒤집어야 한다.
